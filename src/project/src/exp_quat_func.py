...
def compute_gab(g0a,g0b):
    ...
    """
    gab = np.dot(np.linalg.pinv(g0a),g0b)
    return gab
...
def find_omega_theta(R):
    ...
    omega_prefix = 1/(2*np.sin(theta))
    omega_body = np.array([R[2,1] - R[1,2] , R[0,2] - R[2,0], R[1,0] - R[0,1]])
    omega = omega_prefix * omega_body;
    return (omega, theta)
...
def find_v(omega, theta, trans):
    ...
    #r = find_omega_theta(rbt)
    if not omega.shape == (3,):
        omega = np.matrix(omega).T
    if (theta == 0):
        v = trans
    else:
        w = np.matrix(omega).T
        w_hat = kfs.skew_3d(omega.T)
        
        A = np.matrix(np.dot((np.eye(3) - sp.linalg.expm(w_hat*theta)),w_hat) + w*w.T*theta)
        v = np.linalg.pinv(A) * np.matrix(trans).T
