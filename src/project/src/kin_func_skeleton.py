...
def rotation_3d(omega, theta):
    ...
    # define the commonly used stuff
    ...
    # rodriquez formula for matrix exponential
    rot = np.identity(3)
    rot += (omega_hat/omega_norm) * np.sin(omega_norm * theta)
    rot += (np.square(omega_hat)/pow(omega_norm,2)) * (1 - np.cos(omega_norm*theta)) 
    
    
    return sp.linalg.expm(omega_hat*theta)
    #return rot
...
def hat_3d(xi):
    ...
    xi_hat = np.hstack((w_hat,v))   # add a column of transpose

    xi_hat = np.vstack((xi_hat,zeros)) # add a row of 0s at the end
...
def homog_2d(xi, theta):
    ...
    w  = xi[2]
    omega_theta = w * theta
    R = rotation_2d(omega_theta)

    p = np.array([[1-np.cos(omega_theta),np.sin(omega_theta)],
                  [-np.sin(omega_theta), 1 - np.cos(omega_theta)]])


    p = p.dot(np.array( [[0, -1],[1, 0]] ))

    p = p.dot(np.array([vx/w, vy/w]))

    p = np.matrix(p).T 
 
  
    row3 = np.array([0,0,1]) # value of the last row
